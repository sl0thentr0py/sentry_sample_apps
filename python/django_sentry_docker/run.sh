...
PROJECT_NAME=django_sentry_docker
...
function setup_local_files() {
  if [ ! -f "./configs/uwsgi.ini" ]; then
    ...
    gsed -i '1,/PROJECT_NAME=/{x;/first/s///;x;s/PROJECT_NAME=/PROJECT_NAME='$1'/;}' ./run.sh
    gsed -i '1,/module=/{x;/first/s///;x;s/module=/module='$1'.wsgi:application/;}' ./configs/uwsgi.ini
    gsed -i '1,/DJANGO_SETTINGS_MODULE=/{x;/first/s///;x;s/DJANGO_SETTINGS_MODULE=/DJANGO_SETTINGS_MODULE='$1'.settings/;}' ./envs/.env.local
    gsed -i'' -e '1,/ADDITIONAL_DOCKER_COMPOSE_PARAMS/{x;/first/s///;x;s/#ADDITIONAL_DOCKER_COMPOSE_PARAMS/ADDITIONAL_DOCKER_COMPOSE_PARAMS/;}' ./run.sh
  fi
}
...
case $1 in
  ...
  if docker run --rm -v $(pwd)/app:/code -w /code -e DEFAULT_PERMS=$(id -u):$(id -g) python:3-slim /bin/bash -c "pip install \"$DJANGO_VERSION\" && django-admin startproject ${2} .&& chown -R \$DEFAULT_PERMS /code" ; then
    ...
    # Command failed
    gsed "/${DJANGO_VERSION}/d" ./docker/requirements.txt > temp_req && mv temp_req ./docker/requirements.txt
  fi 
  ...
  #gsed -i 's|DJANGO_SECRET_KEY=""|DJANGO_SECRET_KEY="'"$DJANGO_SECRET_KEY"'"|g' ./envs/.env.local
  gsed -i'' -e 's/DJANGO_SECRET_KEY=\"\"/DJANGO_SECRET_KEY=\"'$DJANGO_SECRET_KEY'\"/g' ./envs/.env.local
